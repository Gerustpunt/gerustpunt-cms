...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE INDEX \`posts_meta_meta_og_image_idx\` ON \`posts\` (\`meta_og_image_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`posts_rels_authors_id_idx\` ON \`posts_rels\` (\`authors_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`_posts_v_version_faqs\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`question\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_posts_v_version_faqs_order_idx\` ON \`_posts_v_version_faqs\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`_posts_v_version_faqs_parent_id_idx\` ON \`_posts_v_version_faqs\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`_posts_v\` (
  	...
  	\`version_focus_keyword\` text,
  	\`version_canonical_override\` text,
  	\`version_noindex\` integer DEFAULT false,
  	\`version_schema_type\` text DEFAULT 'BlogPosting',
  	\`version_meta_title\` text,
  	\`version_meta_description\` text,
  	\`version_meta_image_id\` integer,
  	\`version_meta_og_image_id\` integer,
  	\`version_meta_twitter_creator\` text,
  	\`version_slug\` text,
  ...
  await db.run(sql`CREATE INDEX \`_posts_v_version_meta_version_meta_og_image_idx\` ON \`_posts_v\` (\`version_meta_og_image_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`_posts_v_rels_authors_id_idx\` ON \`_posts_v_rels\` (\`authors_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`_posts_v_version_faqs\`;`)
